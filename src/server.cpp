...
void* User::userConnectionLoop(void *param)
{
    ...
    while (1)
    {
        ...
        {
        case DATA:
        case UPLD:  (*info.ref).upload();
        case DWNL:  (*info.ref).download();
        case DELT:  (*info.ref).del(get<1>(message));
        case LSSV:  (*info.ref).listServer();
        case LSCL:
        case GSDR:  (*info.ref).syncAllUserConnections();
        case DVCE:
